...
- `ZplCommand\FieldOrientation` constructor parameter renamed from `$fieldRotation` to `$orientation` to match the class name and the ZPL spec's "field orientation" terminology. Callers using `new FieldOrientation(fieldRotation: …)` must update to `orientation:`. Positional callers are unaffected. ([`47a3d4a`](https://github.com/JanisVepris/zpl-builder-php/commit/47a3d4a))

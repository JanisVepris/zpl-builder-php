...
- `Enum\FontExtension::TrueTypeExtension` (`.TTE`, TrueType extension/collection) — a third extension alongside `.FNT` and `.TTF`, accepted by `^CW` (`fontIdentifier()`). `^A@` (`fontByName()`) does **not** accept it — see the `FontName` note below.

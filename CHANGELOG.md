...
- `ZplCommand\RawCommand` (and `ZplBuilder::raw()`) now reject empty input with `StringLengthOutOfRangeException`. Previously `raw('')` added a no-op entry to the command list — `getCommands()` length bumped while the rendered ZPL gained nothing. The escape hatch still accepts arbitrarily long fragments. ([`6c4739d`](https://github.com/JanisVepris/zpl-builder-php/commit/6c4739d))

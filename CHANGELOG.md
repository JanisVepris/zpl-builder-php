...
- `ZplBuilder::calibrateRfidTransponder()` and `ZplCommand\CalibrateRfidTransponder` add support for `^HR` (Calibrate RFID Transponder Position), initiating an RFID transponder-position calibration that returns a results table to the host ([`0f2a746`](https://github.com/JanisVepris/zpl-builder-php/commit/0f2a746))
- `ZplBuilder::searchWiredPrintServer()` and `ZplCommand\SearchWiredPrintServer` add support for `^NB` (Search for Wired Print Server during Network Boot), with `Enum\WiredPrintServerCheck` choosing whether to check for a wired print server at boot ([`9a4a286`](https://github.com/JanisVepris/zpl-builder-php/commit/9a4a286))
- `ZplBuilder::networkId()` and `ZplCommand\NetworkId` add support for `^NI` (Network ID Number), assigning the printer's RS-485 network ID (1 to 999) ([`0bb1a81`](https://github.com/JanisVepris/zpl-builder-php/commit/0bb1a81))
- `ZplBuilder::setSnmp()` and `ZplCommand\SetSnmp` add support for `^NN` (Set SNMP), setting the system name/contact/location and get/set/trap community names ([`f3c6e9e`](https://github.com/JanisVepris/zpl-builder-php/commit/f3c6e9e))
- `ZplBuilder::primaryDevice()` and `ZplCommand\PrimaryDevice` add support for `^NP` (Set Primary/Secondary Device), with `Enum\NetworkDevice` choosing whether the printer's or print server's network settings are used at boot ([`f43f3e3`](https://github.com/JanisVepris/zpl-builder-php/commit/f43f3e3))
- `ZplBuilder::wiredNetworkSettings()` and `ZplCommand\WiredNetworkSettings` add support for `^NS` (Change Wired Networking Settings), with `Enum\IpResolution` selecting IP resolution plus address/subnet/gateway and optional WINS/timeout/ARP/port parameters ([`c28dc0f`](https://github.com/JanisVepris/zpl-builder-php/commit/c28dc0f))
- `ZplBuilder::setSmtp()` and `ZplCommand\SetSmtp` add support for `^NT` (Set SMTP), setting the SMTP server address and print-server domain for e-mail alerts ([`bfb492e`](https://github.com/JanisVepris/zpl-builder-php/commit/bfb492e))
- `ZplBuilder::webAuthTimeout()` and `ZplCommand\WebAuthenticationTimeout` add support for `^NW` (Set Web Authentication Timeout Value), setting the printer web-page password timeout (0 to 255 minutes) ([`6b9834d`](https://github.com/JanisVepris/zpl-builder-php/commit/6b9834d))
- `ZplBuilder::readAfiOrDsfidByte()` and `ZplCommand\ReadAfiOrDsfidByte` add support for `^RA` (Read AFI or DSFID Byte), with `Enum\RfidByteFormat`, `Enum\RfidMotion`, and `Enum\RfidByteType` selecting the output format, label motion, and byte to read ([`2062b5a`](https://github.com/JanisVepris/zpl-builder-php/commit/2062b5a))
- `ZplBuilder::defineEpcDataStructure()` and `ZplCommand\DefineEpcDataStructure` add support for `^RB` (Define EPC Data Structure), defining the total bit size and per-partition bit sizes for EPC tag encoding ([`d8a843e`](https://github.com/JanisVepris/zpl-builder-php/commit/d8a843e))
- `ZplBuilder::enableEasBit()` and `ZplCommand\EnableEasBit` add support for `^RE` (Enable/Disable E.A.S. Bit), toggling the Electronic Article Surveillance bit on supported ISO15693 tags ([`7b779cb`](https://github.com/JanisVepris/zpl-builder-php/commit/7b779cb))
- `ZplBuilder::readWriteRfidFormat()` and `ZplCommand\ReadWriteRfidFormat` add support for `^RF` (Read or Write RFID Format), with `Enum\RfidOperation` and `Enum\RfidReadWriteFormat` selecting the read/write operation and data format ([`f8e6efc`](https://github.com/JanisVepris/zpl-builder-php/commit/f8e6efc))
- `ZplBuilder::getRfidTagId()` and `ZplCommand\GetRfidTagId` add support for `^RI` (Get RFID Tag ID), reading a tag's serial number into a field, with `Enum\RfidDataOrder` selecting byte order ([`74b5f7e`](https://github.com/JanisVepris/zpl-builder-php/commit/74b5f7e))
- `ZplBuilder::enableRfidMotion()` and `ZplCommand\EnableRfidMotion` add support for `^RM` (Enable RFID Motion), toggling whether the label moves when it reaches the program position ([`d722a04`](https://github.com/JanisVepris/zpl-builder-php/commit/d722a04))
- `ZplBuilder::detectMultipleRfidTags()` and `ZplCommand\DetectMultipleRfidTags` add support for `^RN` (Detect Multiple RFID Tags in Encoding Field), toggling the multiple-tag check before reading or writing ([`4ed0dd2`](https://github.com/JanisVepris/zpl-builder-php/commit/4ed0dd2))
- `ZplBuilder::rfidBlockRetries()` and `ZplCommand\RfidBlockRetries` add support for `^RR` (Specify RFID Retries for a Block), setting how many times the printer retries a single tag block (0 to 10) ([`053cd3a`](https://github.com/JanisVepris/zpl-builder-php/commit/053cd3a))
- `ZplBuilder::setUpRfidParameters()` and `ZplCommand\SetUpRfidParameters` add support for `^RS` (Set Up RFID Parameters), configuring tag type, transponder position, void length, retry count, and error handling, with `Enum\RfidErrorHandling` and `Enum\ApplicatorSignal` ([`3bdd551`](https://github.com/JanisVepris/zpl-builder-php/commit/3bdd551))
- `ZplBuilder::readRfidTag()` and `ZplCommand\ReadRfidTag` add support for `^RT` (Read RFID Tag), reading tag block data into a field (legacy command; `readWriteRfidFormat()` is preferred) ([`937aa7b`](https://github.com/JanisVepris/zpl-builder-php/commit/937aa7b))
- `ZplBuilder::setRfidPowerLevels()` and `ZplCommand\SetRfidPowerLevels` add support for `^RW` (Set RFID Read and Write Power Levels), with `Enum\RfidPowerLevel` selecting the read and write power and an optional antenna port ([`fb3dc30`](https://github.com/JanisVepris/zpl-builder-php/commit/fb3dc30))
- `ZplBuilder::setRfidTagPassword()` and `ZplCommand\SetRfidTagPassword` add support for `^RZ` (Set RFID Tag Password and Lock Tag), with `Enum\RfidPasswordMemoryBank` and `Enum\RfidLockStyle` selecting the Gen 2 memory bank and lock style ([`3139ec8`](https://github.com/JanisVepris/zpl-builder-php/commit/3139ec8))
- `ZplBuilder::setAntennaParameters()` and `ZplCommand\SetAntennaParameters` add support for `^WA` (Set Antenna Parameters), with `Enum\Antenna` selecting the receive and transmit antenna ([`363f6d1`](https://github.com/JanisVepris/zpl-builder-php/commit/363f6d1))
- `ZplBuilder::printDirectoryLabel()` and `ZplCommand\PrintDirectoryLabel` add support for `^WD` (Print Directory Label), listing bar codes/fonts/objects on a device, with `Enum\DirectoryDevice` selecting the device (including resident objects) ([`5dfa0ba`](https://github.com/JanisVepris/zpl-builder-php/commit/5dfa0ba))
- `ZplBuilder::setWepMode()` and `ZplCommand\SetWepMode` add support for `^WE` (Set WEP Mode), configuring WEP encryption with `Enum\WepEncryptionMode`, `Enum\WepAuthenticationType`, and `Enum\WepKeyStorage` plus up to four encryption keys ([`cd048f4`](https://github.com/JanisVepris/zpl-builder-php/commit/cd048f4))
- `ZplBuilder::encodeAfiOrDsfidByte()` and `ZplCommand\EncodeAfiOrDsfidByte` add support for `^WF` (Encode AFI or DSFID Byte), with `Enum\RfidWriteProtect` controlling write protection alongside the motion/format/byte-type enums ([`5a1125a`](https://github.com/JanisVepris/zpl-builder-php/commit/5a1125a))
- `ZplBuilder::changeWirelessNetworkSettings()` and `ZplCommand\ChangeWirelessNetworkSettings` add support for `^WI` (Change Wireless Network Settings), mirroring `^NS` with `Enum\IpResolution` and optional trailing WINS/timeout/ARP/port parameters ([`042499b`](https://github.com/JanisVepris/zpl-builder-php/commit/042499b))
- `ZplBuilder::setLeapParameters()` and `ZplCommand\SetLeapParameters` add support for `^WL` (Set LEAP Parameters), enabling Cisco LEAP authentication with a username and password, with `Enum\LeapMode` toggling the mode ([`b7a3f0f`](https://github.com/JanisVepris/zpl-builder-php/commit/b7a3f0f))
- `ZplBuilder::setWirelessPassword()` and `ZplCommand\SetWirelessPassword` add support for `^WP` (Set Wireless Password), setting the four-digit wireless print-server password (emitted zero-padded) ([`b860943`](https://github.com/JanisVepris/zpl-builder-php/commit/b860943))
- `ZplBuilder::setTransmitRate()` and `ZplCommand\SetTransmitRate` add support for `^WR` (Set Transmit Rate), toggling the 1/2/5.5/11 Mb/s transmit rates with `Enum\TransmitPower` selecting the power level ([`646b52f`](https://github.com/JanisVepris/zpl-builder-php/commit/646b52f))
- `ZplBuilder::setWirelessCardValues()` and `ZplCommand\SetWirelessCardValues` add support for `^WS` (Set Wireless Card Values), setting the ESSID with `Enum\WirelessOperatingMode` and `Enum\WirelessPreamble` ([`db56417`](https://github.com/JanisVepris/zpl-builder-php/commit/db56417))
- `ZplBuilder::writeRfidTag()` and `ZplCommand\WriteRfidTag` add support for `^WT` (Write/Encode Tag), encoding field data to the current RFID tag with block, retries, motion, write-protect, format, and verify options ([`f67f93d`](https://github.com/JanisVepris/zpl-builder-php/commit/f67f93d))
- `ZplBuilder::verifyRfidEncoding()` and `ZplCommand\VerifyRfidEncoding` add support for `^WV` (Verify RFID Encoding Operation), toggling RFID write verification ([`8b1124f`](https://github.com/JanisVepris/zpl-builder-php/commit/8b1124f))
- `ZplBuilder::networkConnect()` and `ZplCommand\NetworkConnect` add support for `~NC` (Network Connect), selecting an RS-485 printer by its three-digit network ID ([`73eef49`](https://github.com/JanisVepris/zpl-builder-php/commit/73eef49))
- `ZplBuilder::setAllNetworkPrintersTransparent()` and `ZplCommand\SetAllNetworkPrintersTransparent` add support for `~NR` (Set All Network Printers Transparent) ([`cd55118`](https://github.com/JanisVepris/zpl-builder-php/commit/cd55118))
- `ZplBuilder::setConnectedPrinterTransparent()` and `ZplCommand\SetConnectedPrinterTransparent` add support for `~NT` (Set Currently Connected Printer Transparent) ([`08bcb92`](https://github.com/JanisVepris/zpl-builder-php/commit/08bcb92))
- `ZplBuilder::reportRfidEncodingResults()` and `ZplCommand\ReportRfidEncodingResults` add support for `~RV` (Report RFID Encoding Results), with `Enum\RfidReportMode` enabling or disabling result reporting ([`3352595`](https://github.com/JanisVepris/zpl-builder-php/commit/3352595))
- `ZplBuilder::printConfigurationLabel()` and `ZplCommand\PrintConfigurationLabel` add support for `~WC` (Print Configuration Label) ([`e39202c`](https://github.com/JanisVepris/zpl-builder-php/commit/e39202c))
- `ZplBuilder::printNetworkConfigurationLabel()` and `ZplCommand\PrintNetworkConfigurationLabel` add support for `~WL` (Print Network Configuration Label) ([`a696bd4`](https://github.com/JanisVepris/zpl-builder-php/commit/a696bd4))
- `ZplBuilder::resetWirelessCard()` and `ZplCommand\ResetWirelessCard` add support for `~WR` (Reset Wireless Card) ([`67c122d`](https://github.com/JanisVepris/zpl-builder-php/commit/67c122d))

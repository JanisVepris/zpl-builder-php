...
- `(string) $builder` permanently flipped `$formatEnded`, so logging a debug copy mid-build broke the builder. `render()` is now a pure iteration over `$this->commands`; the `$formatEnded` bookkeeping is gone entirely. ([`7424e4b`], [`1f8752c`])
...
- GitHub Actions CI workflow running `cs`, `stan`, and `test` as separate jobs across PHP 8.3 / 8.4 / 8.5. ([`2e25e50`], [`f5c9e8d`])
...
- `ZplBuilder::getCommands()` accessor for test introspection and external rendering. ([`36243bc`])
- `ZplBuilder::hasFontPreset()`, `removeFontPreset()`, `getFontPresets()` for preset registry observability. ([`36243bc`])
- Full unit-test coverage of `ZplBuilder` — every public method now has at least one positive test, 53 tests in `ZplBuilderTest` total. ([`887d4ed`])
...
- `FontSettings` instances are now lazily allocated on first access (via the new private `fontSettingsFor()` helper) instead of pre-creating all 36 cases up front in the constructor and `reset()`. ([`36243bc`])
- `render()` rewritten as `implode(…) . $separator` instead of a manual `.=` loop. Trailing newline behaviour preserved. ([`36243bc`])
- Every public `ZplBuilder` method now has a short docblock describing what it does and which ZPL command it emits. Methods that can throw also document the exception types via `@throws`. ([`436494f`], [`caef0aa`])
- Dropped the `jetbrains/phpstorm-attributes` dev dependency and removed `#[Pure]` from exception constructors. The attribute was IDE-only and didn't earn its keep. ([`caef0aa`])
- PHP-CS-Fixer now enforces alphabetical class member ordering via `ordered_class_elements`. ([`3a49a05`])
...
- `render()` / `(string)$b` no longer auto-appends `^XZ`. Call `->end()` explicitly to finalise the format. `end()` is now a regular fluent method that appends `EndFormat` like any other command. ([`1f8752c`])
- `CommandAfterEndException` removed. The builder no longer guards against mutations after `end()` — composing valid ZPL (e.g. not adding fields after `end()`) is the caller's responsibility. ([`1f8752c`])
...
[`241fbc5`]: https://github.com/JanisVepris/zpl-builder-php/commit/241fbc5
...
[`3a49a05`]: https://github.com/JanisVepris/zpl-builder-php/commit/3a49a05
[`3f3df1b`]: https://github.com/JanisVepris/zpl-builder-php/commit/3f3df1b
[`436494f`]: https://github.com/JanisVepris/zpl-builder-php/commit/436494f
[`60ae6ce`]: https://github.com/JanisVepris/zpl-builder-php/commit/60ae6ce
[`6d1d325`]: https://github.com/JanisVepris/zpl-builder-php/commit/6d1d325
[`6d857e6`]: https://github.com/JanisVepris/zpl-builder-php/commit/6d857e6
[`7424e4b`]: https://github.com/JanisVepris/zpl-builder-php/commit/7424e4b
...
[`a936842`]: https://github.com/JanisVepris/zpl-builder-php/commit/a936842
[`bb41000`]: https://github.com/JanisVepris/zpl-builder-php/commit/bb41000
[`c4810fb`]: https://github.com/JanisVepris/zpl-builder-php/commit/c4810fb
[`caef0aa`]: https://github.com/JanisVepris/zpl-builder-php/commit/caef0aa
[`dabd2b3`]: https://github.com/JanisVepris/zpl-builder-php/commit/dabd2b3
